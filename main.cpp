...
// все задачи выполняюся в формате FILO (First In Last Out) - первая пришла, последняя вышла (Это называется СТЕК - крутая штука)
...
// локальные переменные автоматически уничтожаются по завршении блока.
// все что до функции main называет глобальным пространством имен
